...
import { createReferenceAccount, upsertAiEmployee, upsertBrandProfile, upsertTargetProfile } from "@/features/project-context/actions";
  ...
  return (
    <div className="space-y-6">
      ...
      <form
        ...
        action={(formData) => {
          startTransition(async () => {
            const result = await createReferenceAccount(formData);
            setMessages((current) => ({ ...current, reference: result.message }));
          });
        }}
      >
        <input type="hidden" name="projectId" value={projectId} />
        <SectionHeader
          title="参考アカウント分析"
          description="参考にしたいアカウントの特徴を登録すると、AI社員が投稿案をその方向に寄せて最適化します。丸写しではなく、自社向けに変換して使います。"
        />
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="参考アカウント名">
            <Input name="accountName" placeholder="例: 競合A、ベンチマーク店舗" required />
          </Field>
          <Field label="参考URL">
            <Input name="accountUrl" type="url" placeholder="https://www.instagram.com/..." />
          </Field>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="参考にする理由">
            <Textarea name="reason" placeholder="例: 世界観が近い、導線がうまい、保存されやすい投稿が多い" />
          </Field>
          <Field label="強み">
            <Textarea name="strengths" placeholder="例: 冒頭の見せ方、悩み訴求、事例の出し方が分かりやすい" />
          </Field>
          <Field label="見た目・デザインの特徴">
            <Textarea name="visualNotes" placeholder="例: 明るい写真、余白多め、1枚目に大きな見出し" />
          </Field>
          <Field label="投稿内容・構成の特徴">
            <Textarea name="contentNotes" placeholder="例: 悩み提示→原因→解決策→予約導線の順番" />
          </Field>
          <Field label="真似しない点">
            <Textarea name="avoidNotes" placeholder="例: 強すぎる煽り、過度なBefore/After、派手すぎる装飾" />
          </Field>
          <Field label="自社投稿への取り入れ方">
            <Textarea name="applyNotes" placeholder="例: 冒頭の構成だけ参考にし、語尾はブランドトーンに合わせる" />
          </Field>
        </div>
        <FormFooter message={messages.reference} isPending={isPending} label="参考アカウントを保存" />
      </form>

      <section className="rounded-lg border bg-white p-5">
        <SectionHeader title="登録済み参考アカウント" description="AI社員チャットの回答時に、以下の分析内容が参照されます。" />
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {workspace.referenceAccounts.length === 0 ? (
            <p className="text-sm text-muted-foreground">まだ参考アカウントが登録されていません。</p>
          ) : (
            workspace.referenceAccounts.map((account) => (
              <article key={account.id} className="rounded-md border p-4 text-sm">
                <h3 className="font-semibold">{account.account_name}</h3>
                {account.account_url ? (
                  <a href={account.account_url} target="_blank" rel="noreferrer" className="mt-1 block truncate text-xs text-primary hover:underline">
                    {account.account_url}
                  </a>
                ) : null}
                <p className="mt-3 whitespace-pre-wrap text-xs leading-6 text-muted-foreground">
                  {account.apply_notes || account.strengths || account.reason || "分析メモ未設定"}
                </p>
              </article>
            ))
          )}
